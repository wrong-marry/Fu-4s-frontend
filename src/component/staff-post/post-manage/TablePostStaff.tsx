...
function TablePostStaff({ flag, setFlag }: TablePostStaffProps) {
	const pageSize = 20;
	...
	const confirmRestorePost = async () => {
		if (postToRestore) {
			try {
				const token = localStorage.getItem("token");
				const response = await fetch(
					`http://localhost:8080/api/v1/staff/restoredPost?id=${postToRestore.id}`,
					{
						method: "PUT",
						headers: {
							Authorization: `Bearer ${token}`,
	...
	const confirmHiddenPost = async () => {
		if (postToHiden) {
			try {
				const token = localStorage.getItem("token");
				const response = await fetch(
					`http://localhost:8080/api/v1/staff/hidenPost?id=${postToHiden.id}`,
					{
						method: "PUT",
						headers: {
							Authorization: `Bearer ${token}`,
	...
	const confirmDeletePost = async () => {
		if (postToDelete) {
			try {
				const token = localStorage.getItem("token");
				const response = await fetch(
					`http://localhost:8080/api/v1/staff/deletedPost?id=${postToDelete.id}`,
					{
						method: "PUT",
						headers: {
							Authorization: `Bearer ${token}`,
