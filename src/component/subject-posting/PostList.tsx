...
import {
  POST_PAGE_SIZE,
  SearchRequest,
} from "../../page/search/SearchPage.tsx";
...
function PostList(props: { searchRequest: SearchRequest }) {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        let api =
          `${BASE_URL}/api/v1/post/getAllPost?pageSize=` + POST_PAGE_SIZE;
        if (searchRequest.isTest != null)
          api += `&isTest=${searchRequest.isTest}`;
        if (searchRequest.subjectCode)
          api += `&subjectCode=${searchRequest.subjectCode}`;
          ...
          <Table>
            ...
            <Table.Tbody>
              ...
              {posts.length ? (
                ...
                <Table.Tr>
                  <Table.Td>No posts found.</Table.Td>
                </Table.Tr>
              )}
            </Table.Tbody>
          </Table>
