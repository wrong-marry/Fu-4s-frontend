...
function TableSubject() {
	...
	useEffect(() => {
		const fetchAllSubject = async () => {
			...
			try {
				const response = await fetch(`${BASE_URL}/api/v1/admin/getNumSubject`, {
					headers: {
	...
	const handleSaveClick = async () => {
		if (currentSubject) {
			try {
				const token = localStorage.getItem("token");
				const response = await fetch(`${BASE_URL}/api/v1/admin/createSubject`, {
					method: "POST",
					headers: {
						Authorization: `Bearer ${token}`,
						"Content-Type": "application/json",
	...
	const subjectsList = filteredSubjects.map((subject) => (
		<Table.Tr className="text-xs">
			<Table.Td className="py-5 px-6 font-medium">
				<Anchor href={`/subject/${subject.code}`}>{subject.code}</Anchor>
			</Table.Td>
			<Table.Td className="font-medium">
				<Anchor href={`/subject/${subject.code}`}>{subject.name}</Anchor>
			</Table.Td>
	...
	return (
		<Card shadow="lg" withBorder radius="md">
			<div className="container px-4 mx-auto ">
				<div>
					<h3 className="text-xl font-bold w-auto p-5">SUBJECT STATISTICS</h3>
				</div>
			...
			<Card radius="md" shadow="md" withBorder pt="md" mt="lg">
				<div className="container px-4 mx-auto">
					<div className="px-6 border-b">
						<Grid m="md">
							<Grid.Col span={3}>
								<h3 className="text-lg font-bold mt-1">SUBJECT MANAGEMENT</h3>
							</Grid.Col>
							<Grid.Col span={9}>
