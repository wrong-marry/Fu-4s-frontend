...
function Navbar() {
    ...
    useEffect(() => {
        ...
        if (["ADMIN", "STAFF"].includes(data?.role ?? "")) localStorage.setItem('role', data?.role);
    }, [data]);
    ...
    return (
        <>
            <header
                className="w-full h-16 flex items-center justify-between sticky top-0 z-20 shadow-sm bg-[--mantine-color-body]">
