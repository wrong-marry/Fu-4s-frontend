...
export function EditLearningMaterialForm() {
    ...
    const handleDownloadOldFile = async (filename: string) => {
        const response = await fetch(`http://3.27.235.175:8080/api/v1/learningMaterial/getFile?id=${id}&filename=${filename}`);
    ...
    const handleRemove = () => {
        fetch(`http://3.27.235.175:8080/api/v1/learningMaterial/remove?id=${id}&username=${localStorage.getItem('username')}`, {
            method: "DELETE"
        }).then(() => {
