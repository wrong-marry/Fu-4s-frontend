...
export function UserPostTable() {
    ...
    useEffect(() => {
        const fetchPost = async () => {
            ...
            try {
                const response = await fetch(
                    `https://api.fu4s.online.175:8080/api/v1/post/getAllByUsername?username=${username}&pageNum=${activePage}&pageSize=${pageSize}`
                );
        ...
        const fetchNum = async () => {
            ...
            try {
                const response = await fetch(
                    `https://api.fu4s.online.175:8080/api/v1/post/getNum?username=${username}`
                );
                const data = await response.json();
            ...
            <tr className="text-xs bg-gray-50" key={post.id}>
                ...
                <td style={{textAlign: "center"}}>
                    <span className="inline-block py-1 px-2 text-white bg-gray-600 rounded-full">
                            {post.test ? <a href={"/edit-mock-test/" + post.id}>Edit</a> :
