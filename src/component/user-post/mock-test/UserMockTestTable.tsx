...
export function UserMockTestTable() {
  ...
  useEffect(() => {
    const fetchPost = async () => {
      ...
      try {
        const response = await fetch(
          `${BASE_URL}/api/v1/questionSet/getAllByUsername?username=${username}&pageNum=${activePage}&pageSize=${pageSize}`
        );
    ...
    const fetchNum = async () => {
      ...
      try {
        const response = await fetch(
          `${BASE_URL}/api/v1/questionSet/getNum?username=${username}`
        );
        const data = await response.json();
