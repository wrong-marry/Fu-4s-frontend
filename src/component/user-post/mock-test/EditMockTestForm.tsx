...
export function EditMockTestForm() {
    ...
    useEffect(() => {
        const fetchSubject = async () => {
            try {
                const response = await fetch(`http://3.27.235.175:8080/api/v1/subject/getAll`);
        ...
        const fetchMockTest = async () => {
            try {
                const response = await fetch(`http://3.27.235.175:8080/api/v1/questionSet/getById?id=` + id);
                const data = await response.json();
                setTest(data);
    ...
    const handleEdit = () => {
        ...
        fetch(`http://3.27.235.175:8080/api/v1/questionSet/edit?id=${id}&title=${title}&subjectCode=${subject}&username=${localStorage.getItem('username')}`, {
    ...
    const handleRemove = () => {
        fetch(`http://3.27.235.175:8080/api/v1/questionSet/remove?id=${id}&username=${localStorage.getItem('username')}`, {
            method: "DELETE"
        }).then(() => {
            navigate(`/user/post/mock-test`);
        })
